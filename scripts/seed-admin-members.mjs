 ...
 * Safe to re-run — checks before inserting members.
 */
import pg from "pg";
...
for (const admin of newAdmins) {
  ...
  if (existing.length > 0) {
    ...
    if (existing[0].role !== "admin" && existing[0].role !== "owner") {
      await conn.query("UPDATE users SET role = 'admin' WHERE id = $1", [userId]);
  ...
  const { rows: existingMember } = await conn.query(
    "SELECT id, \"memberNumber\" FROM members WHERE \"userId\" = $1",
    [userId]
  );
  ...
  if (existingMember.length > 0) {
    ...
    await conn.query(
      `INSERT INTO members ("userId", "memberNumber", tier, "joinDate", "renewalDate", active, notes)
       VALUES ($1, $2, 'founding', '2024-01-01', '2027-01-01', true, $3)`,
...
const { rows: users } = await conn.query(
  `SELECT u.id, u.email, u.name, u.role, m."memberNumber", m.tier, m.active
   FROM users u LEFT JOIN members m ON m."userId" = u.id ORDER BY u.id`
);
console.log("\n=== FINAL STATE ===");
console.table(users);
