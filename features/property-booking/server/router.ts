    ...
    properties: router({
      ...
      create: protectedProcedure
        .input(z.object({
          ...
          types: z.array(z.enum(["stand", "blind", "field", "pond", "creek", "food_plot", "zone", "lodge"])).optional(),
          // Keep old single-type field for backwards compat
          type: z.enum(["stand", "blind", "field", "pond", "creek", "food_plot", "zone", "lodge"]).optional(),
          ...
          advanceNoticeHours: z.number().int().min(0).default(0),
        ...
        .mutation(async ({ ctx, input }: { ctx: any; input: any }) => {
          ...
          const ts = now();
          // Build types array: use input.types if provided, otherwise wrap input.type in array for backwards compat
          const types = input.types ?? (input.type ? [input.type] : ["stand"]);

          const result = await db.insert(huntingProperties).values({
            name: input.name,
            slug: input.slug,
            shortName: input.shortName ?? null,
            types,  // Use computed types array
            ...
            overnightExclusive: input.overnightExclusive ?? false,
            advanceNoticeHours: input.advanceNoticeHours ?? 0,
            active: input.active ?? true,
            featuredOnPublicSite: input.featuredOnPublicSite ?? true,
